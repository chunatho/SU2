...
//	const double B = 1 / (2.27e-6);			// "temperature" of the system
	...
	const int a = 100;				// iterations per dipole
	...
	//2 Dimensional Vector containing d-D Lattice Links
        //s[i][j] i house link direction, j houses site number
...
#pragma omp section 
{
	#pragma omp parallel for
			for (int j = 0; j < static_cast<int>(pow(size, d) - 2)/2; j++) {
				//Reset Parameters for good measure
                                index = ((2*j/t)%2 + (2*j/z)%2 + (2*j/y)%2)%2 +2*j;
...
#pragma omp section 
{
	#pragma omp parallel for
			for (int j = 0; j < static_cast<int>(pow(size, d) - 2)/2; j++) {
			   //Reset Parameters for good measure
                           index = ((2*j/t)%2 + (2*j/z)%2 + (2*j/y)%2 + 1)%2 + 2*j;
			   StapleTotal.Coord[0] = StapleTotal.Coord[1] = StapleTotal.Coord[2] = StapleTotal.Coord[3] = 0;
...
double CalculateNewA0(double B, double k) {
	bool test = false;
	double c = 0; double newA0 = 0; double r = 0;

	while (test == false) {
		c = exp(-2 * B*k) + (1 - exp(-2 * B*k))*(double)rand() / (double)RAND_MAX;
		newA0 = 1 + log(c) / (B*k);
